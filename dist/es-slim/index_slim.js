...
* @returns {Solution}
*/
function new_solution(conf) {
    try {
        const retptr = wasm.__wbindgen_add_to_stack_pointer(-16);
        wasm.new_solution(retptr, addHeapObject(conf));
...
class CrosswordClient {
    ...
    generate_crossword_solution = (conf) => {
        return new_solution(conf);
    };
    generate_crossword_puzzle = (conf, puzzle_type) => {
        return new_puzzle(conf, puzzle_type);
    };
    is_puzzle_complete = (puzzle_container) => {
        return is_puzzle_complete(puzzle_container);
    };
    wrong_answers_and_solutions = (puzzle_container) => {
        return wrong_answers_and_solutions(puzzle_container);
    };
    guess_word = (puzzle_container, guess) => {
        return guess_word(puzzle_container, guess);
    };
    remove_answer = (puzzle_container, placement) => {
        return remove_answer(puzzle_container, placement);
    };
}

export { CrosswordClient };
